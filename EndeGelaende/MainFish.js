...
(function (EndeGelaende) {
    class Mainfish extends EndeGelaende.MovingObject {
        ...
        moving() {
            ...
            else if (EndeGelaende.upKey == true && EndeGelaende.object[0].y > 0) { // GRENZEN FUNKTIONIEREN NOCH NCHT
                this.y -= 10;
            }
        ...
        collision() {
            EndeGelaende.object.forEach(function (element) {
                ...
                if (element instanceof EndeGelaende.Fisch) {
                    if ((Math.abs(element.x - EndeGelaende.object[0].x) < 70) && (Math.abs(element.y - EndeGelaende.object[0].y) < 70)) {
                        //alert("AAA");
                        //this.removeAll();
